...
public final class TradeOfferManager {
    ...
    /** Create an offer. {@code offered} must already be removed from the creator (escrowed by the GUI). */
    public static boolean createOffer(ServerPlayerEntity creator, ItemStack offered, long price,
                                      ItemStack requested, String targetName) {
        ...
        TradeOffer offer = new TradeOffer(UUID.randomUUID(), creator.getUuid(),
                creator.getName().getString(), targetName, offered.copy(), price,
                requested == null ? ItemStack.EMPTY : requested.copy(),
                creator.getWorld().getTime());
        state.add(offer);
    ...
    public static boolean accept(ServerPlayerEntity accepter, UUID offerId) {
        ...
        if (offer.requestsItem()) {
            int have = countInInventory(accepter, offer.requestedItem());
            if (have < offer.requestedItem().getCount()) {
                accepter.sendMessage(Text.literal("You need " + offer.requestedItem().getCount() + "x "
                        + offer.requestedItem().getName().getString() + " for this trade.").formatted(Formatting.RED), false);
                return false;
            }
        }
        ...
        if (offer.requestsItem()) {
            removeFromInventory(accepter, offer.requestedItem(), offer.requestedItem().getCount());
            deliverToCreator(server, offer, offer.requestedItem().copy());
        }

        // Hand the escrowed items to the accepter.
        giveOrDrop(accepter, offer.offered().copy());
        accepter.playSound(SoundEvents.ENTITY_PLAYER_LEVELUP, 0.8F, 1.2F);
        accepter.sendMessage(Text.literal("Trade complete — received ")
                .append(offer.offered().getName().copy().formatted(Formatting.YELLOW))
    ...
    /** Cancel an offer and return the escrowed items to the creator. */
    public static boolean cancel(ServerPlayerEntity creator, UUID offerId) {
        ...
        giveOrDrop(creator, offer.offered().copy());
        state.remove(offerId);
